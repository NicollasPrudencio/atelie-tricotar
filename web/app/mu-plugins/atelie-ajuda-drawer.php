...
class Atelie_Ajuda_Drawer {
	 ...
	 * @param string             $caminho_docs Caminho relativo no site de documentação, ex. "/novo-produto/".
	 */
	public static function render( string $titulo, array $dicas, string $caminho_docs ): void {
		self::registrar_assets();

		$id       = 'atelie-ajuda-' . sanitize_title( $titulo );
		$url_docs = rtrim( ATELIE_DOCS_BASE_URL, '/' ) . '/' . ltrim( $caminho_docs, '/' );
		?>
		<button
			type="button"
