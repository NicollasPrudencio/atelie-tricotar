...
add_action('init', function (): void {
    register_post_type('atelie_orcamento', [
    ...
    register_post_type('atelie_materia_prima', [
    ...
    register_post_type('atelie_artesa', [
        ...
        function (WP_Post $post): void {
            ...
            <p>
                ...
                <select name="atelie_mp_unidade" style="width:100%">
                    <?php foreach (['unidade' => 'Unidade', 'metro' => 'Metro', 'grama' => 'Grama', 'novelo' => 'Novelo', 'outro' => 'Outro'] as $valor => $rotulo) : ?>
...
add_action('save_post_atelie_materia_prima', function (int $post_id): void {
...
add_action('save_post_atelie_artesa', function (int $post_id): void {
...
function atelie_orcamento_render_meta_box(WP_Post $post): void
{
    ...
    $artesas = get_posts(['post_type' => 'atelie_artesa', 'numberposts' => -1, 'orderby' => 'title', 'order' => 'ASC']);
    $materias = get_posts(['post_type' => 'atelie_materia_prima', 'numberposts' => -1, 'orderby' => 'title', 'order' => 'ASC']);
    ...
    <div id="atelie-orcamento-app"
        ...
        <p>
            ...
            <select name="atelie_orc_porte" id="atelie-orc-porte">
                <?php foreach (['pequeno' => 'Pequeno', 'medio' => 'Médio', 'grande' => 'Grande'] as $valor => $rotulo) : ?>
...
add_action('save_post_atelie_orcamento', function (int $post_id): void {
    ...
    $horas_reais_raw = isset($_POST['atelie_orc_horas_reais']) ? trim((string) wp_unslash($_POST['atelie_orc_horas_reais'])) : '';
    ...
    $itens_mp = isset($_POST['atelie_orc_item_mp']) && is_array($_POST['atelie_orc_item_mp']) ? $_POST['atelie_orc_item_mp'] : [];
    $itens_qtd = isset($_POST['atelie_orc_item_qtd']) && is_array($_POST['atelie_orc_item_qtd']) ? $_POST['atelie_orc_item_qtd'] : [];
    ...
    foreach ($itens_mp as $i => $mp_id) {
        ...
        $itens_salvos[] = ['materia_prima_id' => $mp_id, 'quantidade' => $qtd];
    }
 ...
 */
add_action('wp_ajax_atelie_sugerir_horas', function (): void {
    ...
    if ($categoria_id <= 0 || $porte === '') {
        wp_send_json_success(['media' => null, 'quantidade' => 0]);
    }
    ...
    $orcamentos = get_posts([
        ...
        'meta_query' => [
            ['key' => '_atelie_orc_porte', 'value' => $porte],
            ['key' => '_atelie_orc_horas_reais', 'compare' => 'EXISTS'],
        ],
    ]);
