<?php
/**
 * Criacao em massa: cria um produto rascunho por foto selecionada e agenda
 * o processamento de cada um separadamente via Action Scheduler — nao e
 * um job so pra tudo, e o que permite revisar o que ja terminou enquanto
 * o resto ainda processa. Ver plano, secao "Criacao de produtos em massa
 * via IA".
 */
...
class Atelie_Lote_Controller
{
    ...
    private function limite_por_lote(): int
    {
    ...
    public function criar_lote(): void
    {
        ...
        foreach ($fotos_ids as $foto_id) {
            ...
            set_post_thumbnail($produto_id, $foto_id);
            ...
            update_post_meta($produto_id, '_atelie_lote_foto_id', $foto_id);
    ...
    {
        $caminho_foto = get_attached_file((int) get_post_meta($produto_id, '_atelie_lote_foto_id', true));

        try {
            $servico = Atelie_Ai_Vision_Service_Factory::criar();
            $sugestao = $servico->analisar($caminho_foto ? [$caminho_foto] : []);
