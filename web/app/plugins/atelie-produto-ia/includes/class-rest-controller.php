...
class Atelie_Rest_Controller {
		...
		add_action(
			...
			function (): void {
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'analisar_fotos' ),
						'permission_callback' => array( $this, 'usuario_pode_criar_produto' ),
						'args'                => array(
							'fotos'             => array( 'required' => true ),
							'receita_imagem_id' => array( 'required' => false ),
							'receita_texto'     => array( 'required' => false ),
						),
					)
				);
				...
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'sugerir_case' ),
						'permission_callback' => array( $this, 'usuario_pode_criar_case' ),
						'args'                => array(
							'fotos'  => array( 'required' => true ),
							'relato' => array( 'required' => false ),
						),
					)
				);
				...
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'editar_imagem' ),
				...
				register_rest_route(
					...
					array(
						'methods'             => 'GET',
						'callback'            => array( $this, 'drive_listar' ),
						'permission_callback' => array( $this, 'usuario_pode_criar_produto' ),
						'args'                => array(
							'pasta' => array( 'required' => false ),
						),
					)
				);
				...
				register_rest_route(
					...
					array(
						'methods'             => 'POST',
						'callback'            => array( $this, 'drive_baixar_fotos' ),
						'permission_callback' => array( $this, 'usuario_pode_criar_produto' ),
						'args'                => array(
							'fotos' => array( 'required' => true ),
						),
	...
	public function analisar_fotos( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( empty( $fotos_ids ) ) {
			return new WP_REST_Response( array( 'erro' => 'Anexe pelo menos uma foto.' ), 400 );
		}

		if ( count( $fotos_ids ) > self::LIMITE_FOTOS_POR_PRODUTO ) {
			return new WP_REST_Response(
				array( 'erro' => 'Máximo de ' . self::LIMITE_FOTOS_POR_PRODUTO . ' fotos por produto.' ),
				400
			);
		}
		...
		try {
			...
			return new WP_REST_Response(
				array( 'erro' => 'Não foi possível gerar a sugestão agora. Pode preencher os campos manualmente.' ),
				502
			);
		}
		...
		return new WP_REST_Response( array( 'sugestao' => $sugestao ), 200 );
	}
	...
	public function drive_listar( WP_REST_Request $request ): WP_REST_Response {
		if ( ! Atelie_Drive_Config::conectado() ) {
			return new WP_REST_Response( array( 'erro' => 'Google Drive não conectado.' ), 400 );
		}
		...
		return new WP_REST_Response( array( 'itens' => $itens ), 200 );
	}
	...
	public function drive_baixar_fotos( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( empty( $resultado ) ) {
			return new WP_REST_Response( array( 'erro' => 'Não deu pra baixar nenhuma das fotos selecionadas.' ), 502 );
		}
	...
	public function editar_imagem( WP_REST_Request $request ): WP_REST_Response {
		...
		if ( ! $foto_id || $prompt === '' ) {
			return new WP_REST_Response( array( 'erro' => 'Selecione a foto e descreva a edição desejada.' ), 400 );
		}
		...
		try {
			...
			return new WP_REST_Response( array( 'erro' => 'Não foi possível editar a imagem agora.' ), 502 );
		}
	...
	private function registrar_log( string $mensagem ): void {
		$log = get_option( 'atelie_ia_log', array() );
		if ( ! is_array( $log ) ) {
			$log = array();
		}
		$log[] = array(
